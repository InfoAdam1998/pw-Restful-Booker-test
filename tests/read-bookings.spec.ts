...
/**
 * GET /ping and GET /booking(/:id) — read-only endpoints. No auth token needed.
 */
test.describe('Booking API - read', { tag: ['@booking', '@read'] }, () => {

    test('GET /ping returns healthy status', { tag: '@smoke' }, async ({ request }) => {
    ...
    test('GET /booking/:id returns the booking you created', { tag: '@smoke' }, async ({ bookingApi }) => {
